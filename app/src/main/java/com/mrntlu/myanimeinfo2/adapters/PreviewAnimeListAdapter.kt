...
import kotlinx.android.synthetic.main.cell_preview_anime.view.*
...
class PreviewAnimeListAdapter(private val interaction: Interaction? = null) : RecyclerView.Adapter<RecyclerView.ViewHolder>() {
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): RecyclerView.ViewHolder {
        return when(viewType){
            ...
            PREVIEW_HOLDER->{
                PreviewAnimeHolder(LayoutInflater.from(parent.context).inflate(R.layout.cell_preview_anime, parent, false), interaction)
            }
            else-> PreviewAnimeHolder(LayoutInflater.from(parent.context).inflate(R.layout.cell_preview_anime, parent, false), interaction)
        }
    }
    ...
    class PreviewAnimeHolder constructor(itemView: View, private val interaction: Interaction?) : RecyclerView.ViewHolder(itemView) {
        fun bind(item: PreviewAnimeResponse) = with(itemView) {
            ...
            itemView.episodesText.text=item.episodes.toString()

            itemView.animeImageProgress.setVisible()
            Glide.with(itemView.context).load(item.image_url).addListener(object:RequestListener<Drawable>{
                override fun onLoadFailed(e: GlideException?, model: Any?, target: Target<Drawable>?, isFirstResource: Boolean): Boolean {
                    itemView.animePreviewImage.setImageResource(R.drawable.ic_no_picture)
                    itemView.animeImageProgress.setGone()
                    return false
                }
            ...
            }).into(itemView.animePreviewImage)
        }
    }
