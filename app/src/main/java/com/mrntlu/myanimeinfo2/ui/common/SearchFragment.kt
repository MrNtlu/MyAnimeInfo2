...
import com.mrntlu.myanimeinfo2.utils.getStringQuery
...
class SearchFragment : Fragment() {
    ...
    private fun setupObservers(query:String){
        if (dataType==MANGA){
            mangaViewModel.getMangaBySearch(query,1).observe(viewLifecycleOwner, Observer {
                printLog(message = it.toString())
                searchMangaAdapter.submitList(it.results)
            })
        }else{
            animeViewModel.getAnimeBySearch(query,1).observe(viewLifecycleOwner, Observer {
                printLog(message = it.toString())
    ...
    private fun setupMangaRecyclerView()=searchRV.apply {
    ...
    private fun setupAnimeRecyclerView()=searchRV.apply {
